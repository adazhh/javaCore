...
public class SingleLinkedList {
    ...
    public void add(Variable obj) {		// 先创建一个结点
        // Node node = new Node(obj); // 把新建的结点加入到链表中
        last.setNext(obj);		// 更改last的指向
        last = obj;		// 更改结点长度
        size++;
    }
    ...
    public void add(int desIndex, Object obj) {		// 判断下标是否合法
        if (desIndex < 0 || desIndex > size+1) { //(+1是因为，允许插入到尾部)
            ...
            Node node = new Node(obj);			// 插入到头结点后一个位置
