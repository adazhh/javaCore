...
public class O28Permutation2 {
    ...
    private void fun(char[] ch,List<String> list,int i){
        ...
        if(i == ch.length-1){
            ...
            //那么j=0，swap(ch,0,0)，就是['a','b','c']，进入递归，自己调自己，只是i为1，
            // 交换(0,0)位置之后的状态我称为 状态B
            //i不等于2，来到这里，j=1，执行第一个swap(ch,1,1)，这个状态我称为
            // 状态C1 ,再进入fun函数，此时标记为T1，i为2，那么这时就进入上一个if，将"abc"放进list中
            /////////////-------》此时结果集为["abc"]
            ...
            for(int j=i;j<ch.length;j++){
                // 第一个swap是交换顺序，第二个是换回来，也就是回退到上一步
                swap(ch,i,j);
                fun(ch,list,i+1);
                swap(ch,i,j);
            }
